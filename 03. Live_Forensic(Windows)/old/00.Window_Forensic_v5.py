...
class MemoryDumper:
    def __init__(self, base_dir):
        ...
        self.winpmem_path = os.path.join(os.path.dirname(__file__), "program", "winpmem.exe")
        
    def dump_physical_memory(self, output_dir):
        try:
            if not os.path.exists(self.winpmem_path):
                print("[!] winpmem.exe 파일을 찾을 수 없습니다. 같은 폴더에 winpmem.exe 파일이 있는지 확인하세요.")
                return False
...
def collect_web_artifacts(folder_name):
    ...
    os.makedirs(web_artifacts_folder, exist_ok=True)
    ...
    if os.path.exists(chrome_history_path):
        ...
        print("Chrome 히스토리 수집 완료...")
    else:
        print("history_path를 발견하지 못하여 생략합니다.")
    ...
    if os.path.exists(chrome_cookies_path):
        ...
        print("Chrome 쿠키 수집 완료...")
    else:
        print("쿠키 path를 발견하지 못하여 생략합니다.")
    ...
    # Internet Explorer 웹 아티팩트 수집
    ie_folder = os.path.join(web_artifacts_folder, "Internet Explorer")
    os.makedirs(ie_folder, exist_ok=True)
    ...
    if os.path.exists(ie_history_path):
        ...
        print("Internet Explorer 히스토리 수집 완료...")

    ie_cookies_path = os.path.join(os.path.expanduser("~"), "AppData", "Local", "Microsoft", "Windows", "Cookies")
    if os.path.exists(ie_cookies_path):
        shutil.copytree(ie_cookies_path, os.path.join(ie_folder, "IE_Cookies"), dirs_exist_ok=True)
        print("Internet Explorer 쿠키 수집 완료...")
