...
class Logger:
    def __init__(self, filename):
        ...
        self.log = open(filename, "a", encoding="utf-8")  # 로그 파일을 추가 모드로 엽니다.

    def write(self, message):
        self.terminal.write(message)  # cmd 창에 출력
...
# sys.stdout을 Logger 클래스로 재정의하여 출력 내용을 로그 파일과 cmd 창에 모두 출력합니다.
sys.stdout = Logger("forensic_log.txt")
